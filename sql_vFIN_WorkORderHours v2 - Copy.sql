...
FROM (SELECT
  ...
  CASE
    ...
    ELSE WLRUNM + WLRUNL
  END AS PlannedLabor
FROM dbo.vF55HRRPTA) AS x) AS y left join dim_ReportingWorkCenters w on w.WorkCenterNum=y.WLMCU
						left join dim_ReportingDepartments d on d.DepartmentID=w.DepartmentID